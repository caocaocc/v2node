...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/caocaocc/v2node/master/script/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/caocaocc/v2node/master/script/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/v2node -N --no-check-certificate https://raw.githubusercontent.com/caocaocc/v2node/master/script/v2node.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/caocaocc/v2node ---
  ${green}0.${plain} 修改配置
————————————————
