...
install_v2node() {
    ...
    if  [[ -z "$version_param" ]] ; then
        last_version=$(curl -Ls "https://api.github.com/repos/caocaocc/v2node/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        url="https://github.com/caocaocc/v2node/releases/download/${last_version}/v2node-linux-${arch}.zip"
...
EOF
    ...
    curl -o /usr/bin/v2node -Ls https://raw.githubusercontent.com/caocaocc/v2node/main/script/v2node.sh
